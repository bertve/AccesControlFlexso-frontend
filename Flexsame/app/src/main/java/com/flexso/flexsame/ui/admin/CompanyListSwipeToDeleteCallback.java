...
public class CompanyListSwipeToDeleteCallback extends ItemTouchHelper.SimpleCallback {
    ...
    public CompanyListSwipeToDeleteCallback(CompanyListAdapter adapter) {
        ...
        backgroundToLeft = new ColorDrawable(Color.RED);
    }
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (dX < 0) { // Swiping to the left
            ...
            backgroundToLeft.setBounds(itemView.getRight() + ((int) dX) - backgroundCornerOffset,
                    itemView.getTop(), itemView.getRight(), itemView.getBottom());
        } else { // view is unSwiped
            backgroundToLeft.setBounds(0, 0, 0, 0);
        }

        backgroundToLeft.draw(c);
        icon.draw(c);
    }
}
